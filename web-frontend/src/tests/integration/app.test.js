...
jest.mock('../../services/api', () => ({
    ...
    marketDataAPI: {
        getBlockchainData: jest.fn().mockResolvedValue({ data: { success: false } }),
    },
    portfolioAPI: {
        getPortfolio: jest.fn().mockResolvedValue({ data: { success: false, portfolios: [] } }),
    },
    predictionAPI: {
...
describe('App Integration Tests', () => {
    ...
    test('renders homepage by default', async () => {
        render(<App />);
        await waitFor(() => {
            expect(screen.getByText(/Next-Gen Investment Analytics/i)).toBeInTheDocument();
        });
    });
});
