...
const testPredictionEndpoint = async () => {
  try {
    ...
    return { success: false, message: `Prediction endpoint test failed: ${error.message}` };
  }
};
...
const testOptimizationEndpoint = async () => {
  try {
    ...
    return { success: false, message: `Optimization endpoint test failed: ${error.message}` };
  }
};
