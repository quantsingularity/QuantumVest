...
class User(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            ...
            'risk_tolerance': float(self.risk_tolerance) if self.risk_tolerance else None,
...
class Asset(db.Model):
    ...
    # Relationships
    price_history = db.relationship('PriceHistory', backref='asset', lazy='dynamic', cascade='all, delete-orphan')
    portfolio_holdings = db.relationship('PortfolioHolding', backref='asset', lazy='dynamic')
...
class Portfolio(db.Model):
    ...
    # Relationships
    holdings = db.relationship('PortfolioHolding', backref='portfolio', lazy='dynamic', cascade='all, delete-orphan')
    transactions = db.relationship('Transaction', backref='portfolio', lazy='dynamic')
    performance_history = db.relationship('PortfolioPerformance', backref='portfolio', lazy='dynamic', cascade='all, delete-orphan')
...
class PortfolioHolding(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    portfolio_id = db.Column(UUID(as_uuid=True), db.ForeignKey('portfolios.id'), nullable=False, index=True)
...
class Transaction(db.Model):
    ...
    # Timestamps
    executed_at = db.Column(db.DateTime(timezone=True), nullable=False, default=datetime.now(timezone.utc))
    created_at = db.Column(db.DateTime(timezone=True), default=datetime.now(timezone.utc))
...
class PortfolioPerformance(db.Model):
    ...
    __table_args__ = (
        UniqueConstraint('portfolio_id', 'timestamp', name='unique_portfolio_timestamp'),
...
class Alert(db.Model):
    ...
    alert_type = db.Column(db.String(50), nullable=False, index=True)  # price, portfolio, news, system
...
class RiskMetrics(db.Model):
    ...
    __table_args__ = (
        UniqueConstraint('portfolio_id', 'calculation_date', name='unique_portfolio_risk_date'),
    )
...
class ComplianceCheck(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id = db.Column(UUID(as_uuid=True), db.ForeignKey('users.id'), nullable=False, index=True)
    portfolio_id = db.Column(UUID(as_uuid=True), db.ForeignKey('portfolios.id'), index=True)
    ...
    # Timestamps
    checked_at = db.Column(db.DateTime(timezone=True), nullable=False, default=datetime.now(timezone.utc))
