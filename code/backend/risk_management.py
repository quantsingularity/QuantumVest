...
from models import Portfolio, PortfolioHolding, Asset, PriceData, db
import logging
...
class RiskManagementService:
    ...
    @staticmethod
    def calculate_var(portfolio_id: str, confidence_level: float = 0.95, 
                     time_horizon: int = 1, method: str = 'historical') -> Dict:
        ...
        try:
            ...
            if portfolio_returns is None or len(portfolio_returns) < 30:
                return {'success': False, 'error': 'Insufficient historical data for VaR calculation'}
    ...
    @staticmethod
    def calculate_portfolio_metrics(portfolio_id: str, benchmark_symbol: str = 'SPY') -> Dict:
        ...
        try:
            ...
            # Basic statistics
            metrics['mean_return'] = float(np.mean(portfolio_returns) * 252)  # Annualized
            metrics['volatility'] = float(np.std(portfolio_returns) * np.sqrt(252))  # Annualized
            ...
            if metrics['volatility'] > 0:
                metrics['sharpe_ratio'] = float(np.mean(excess_returns) / np.std(portfolio_returns) * np.sqrt(252))
            else:
            ...
            if len(downside_returns) > 0:
                metrics['downside_deviation'] = float(np.std(downside_returns) * np.sqrt(252))
                metrics['sortino_ratio'] = float(metrics['mean_return'] / metrics['downside_deviation'])
            else:
            ...
            if benchmark_returns is not None and len(benchmark_returns) == len(portfolio_returns):
                ...
                if benchmark_variance > 0:
                    ...
                    metrics['correlation_with_benchmark'] = float(np.corrcoef(portfolio_returns, benchmark_returns)[0, 1])
                    
                    # Alpha calculation
                    benchmark_mean = np.mean(benchmark_returns) * 252
                    metrics['alpha'] = float(metrics['mean_return'] - (risk_free_rate + metrics['beta'] * (benchmark_mean - risk_free_rate)))
                else:
                    metrics['beta'] = 0
            ...
            if benchmark_returns is not None:
                ...
                if tracking_error > 0:
                    metrics['information_ratio'] = float(np.mean(active_returns) * 252 / tracking_error)
                else:
    ...
    @staticmethod
    def _get_portfolio_returns(portfolio_id: str, days: int = 252) -> Optional[np.ndarray]:
        ...
        try:
            ...
            for holding in holdings:
                asset_returns = RiskManagementService._get_asset_returns(holding.asset_id, days)
    ...
    @staticmethod
    def _historical_var(returns: np.ndarray, confidence_level: float, time_horizon: int) -> float:
    ...
    @staticmethod
    def _parametric_var(returns: np.ndarray, confidence_level: float, time_horizon: int) -> float:
    ...
    @staticmethod
    def _monte_carlo_var(returns: np.ndarray, confidence_level: float, 
    ...
    @staticmethod
    def _calculate_expected_shortfall(returns: np.ndarray, confidence_level: float, 
