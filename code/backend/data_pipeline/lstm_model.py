...
class LSTMModel:
    ...
    def _prepare_data(self, df: pd.DataFrame, target_col: str = 'close', 
                     sequence_length: int = 60) -> Tuple[np.ndarray, np.ndarray, MinMaxScaler]:
        """
    ...
    def train(self, df: pd.DataFrame, asset_type: str, symbol: str, 
             target_col: str = 'close', sequence_length: int = 60,
        ...
        try:
            ...
            # Define callbacks
            model_path = os.path.join(self.model_dir, f"{asset_type}_{symbol.lower()}_model.h5")
            callbacks = [
            ...
            # Save scaler
            scaler_path = os.path.join(self.model_dir, f"{asset_type}_{symbol.lower()}_scaler.pkl")
            ...
            metadata_path = os.path.join(self.model_dir, f"{asset_type}_{symbol.lower()}_metadata.pkl")
            joblib.dump(metadata, metadata_path)
            
            logger.info(f"Model training completed for {symbol} with validation loss: {metadata['final_val_loss']}")
    ...
    def predict(self, df: pd.DataFrame, target_col: str = 'close', 
        ...
        try:
            ...
            if len(scaled_data) < self.sequence_length:
                logger.error(f"Not enough data points. Need at least {self.sequence_length}, got {len(scaled_data)}")
