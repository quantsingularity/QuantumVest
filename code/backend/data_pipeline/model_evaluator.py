...
class ModelEvaluator:
    ...
    def generate_evaluation_report(self, asset_type: str, symbol: str, 
        ...
        try:
            ...
            report_dir = os.path.join(self.reports_dir, f"{asset_type}_{symbol.lower()}")
    ...
    def _generate_plots(self, y_true: np.ndarray, y_pred: np.ndarray, 
                      dates: List[str], report_dir: str, symbol: str) -> None:
        """
        ...
        try:
            ...
            plt.xlabel('Date')
            plt.ylabel('Value')
            plt.xticks(x[::max(1, len(x)//10)], [x_labels[i] for i in x[::max(1, len(x)//10)]], rotation=45)
            plt.legend()
            plt.grid(True)
            plt.tight_layout()
