...
class PortfolioService:
    ...
    @staticmethod
    def create_portfolio(user_id: str, name: str, description: str = None, 
                        currency: str = 'USD', is_default: bool = False) -> Dict:
    ...
    def get_portfolio_details(portfolio_id: str, user_id: str) -> Dict:
        ...
        try:
            ...
            holdings = db.session.query(PortfolioHolding, Asset).join(
                Asset, PortfolioHolding.asset_id == Asset.id
            ...
            for holding, asset in holdings:
                ...
                if latest_price:
                    ...
                    holding.unrealized_pnl = holding.market_value - (holding.quantity * holding.average_cost)
    ...
    @staticmethod
    def add_transaction(portfolio_id: str, user_id: str, asset_symbol: str,
        ...
        try:
            ...
            if transaction_type.lower() == 'buy':
                ...
                realized_pnl = (Decimal(str(price)) - holding.average_cost) * Decimal(str(quantity))
    ...
    def get_portfolio_performance(portfolio_id: str, user_id: str, 
        ...
        try:
            ...
            performance_data = PortfolioPerformance.query.filter(
                and_(
            ...
            if not performance_data:
                ...
                current_value = PortfolioService._calculate_current_portfolio_value(portfolio_id)
    ...
    @staticmethod
    def optimize_portfolio(portfolio_id: str, user_id: str, 
        ...
        try:
            ...
            if len(holdings) < 2:
                return {'success': False, 'error': 'Need at least 2 assets for optimization'}
            
            # Get historical price data for each asset
            asset_returns = {}
            for holding in holdings:
                returns = PortfolioService._get_asset_returns(holding.asset_id, days=252)
                if returns is not None and len(returns) > 0:
                    asset_returns[holding.asset.symbol] = returns
            
            if len(asset_returns) < 2:
                return {'success': False, 'error': 'Insufficient price data for optimization'}
            ...
            for i, symbol in enumerate(symbols):
                recommendations.append({
                    ...
                    'recommendation': 'buy' if optimal_weights[i] > 0.05 else 'hold'
                })
    ...
    def _get_latest_price(asset_id: str) -> Optional[Decimal]:
        ...
        try:
            latest_price_data = PriceData.query.filter_by(
                asset_id=asset_id
    ...
    @staticmethod
    def _optimize_weights(expected_returns: np.ndarray, cov_matrix: np.ndarray, 
        ...
        try:
            ...
            return_scores = (expected_returns - np.mean(expected_returns)) / np.std(expected_returns)
