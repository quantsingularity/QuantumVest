...
from models import db, User, Portfolio, Asset, PortfolioHolding
...
class TestPortfolioManagement:
    ...
    def test_add_transaction(self, app, client, auth_headers):
        ...
        response = client.post(f'/api/v1/portfolios/{portfolio_id}/transactions',
                             data=json.dumps(data),
                             content_type='application/json',
                             headers=auth_headers)
...
class TestDataEndpoints:
    ...
    def test_search_assets(self, app, client, auth_headers):
        ...
        with app.app_context():
            # Create test assets
            assets = [
                Asset(symbol='AAPL', name='Apple Inc.', asset_type='stock', is_active=True),
                Asset(symbol='BTC', name='Bitcoin', asset_type='crypto', is_active=True),
                Asset(symbol='ETH', name='Ethereum', asset_type='crypto', is_active=True)
            ]
            for asset in assets:
                db.session.add(asset)
            db.session.commit()
