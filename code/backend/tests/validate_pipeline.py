...
def test_prediction_service():
    ...
    if result['success']:
        ...
        logger.error(f"Failed to generate predictions for {symbol}: {result.get('error', 'Unknown error')}")
...
def test_api_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Stock data endpoint failed with status code {response.status_code}")
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Stock prediction endpoint failed with status code {response.status_code}")
    except Exception as e:
