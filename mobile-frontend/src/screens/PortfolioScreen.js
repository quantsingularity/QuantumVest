...
const PortfolioScreen = ({ navigation }) => {
  ...
  const handleOptimizePortfolio = async () => {
    ...
    if (assets.length === 0) {
      Alert.alert(
        "Input Error",
        "Please enter at least one asset symbol, separated by commas.",
      );
      return;
    }
    if (isNaN(riskTolerance) || riskTolerance < 0 || riskTolerance > 1) {
      Alert.alert(
        "Input Error",
        "Risk tolerance must be a number between 0 and 1.",
      );
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        Alert.alert(
          "Optimization Failed",
          response.data.error || "Could not optimize portfolio.",
        );
      }
    } catch (error) {
      ...
      Alert.alert(
        "Error",
        "Failed to optimize portfolio. Check API connection.",
      );
    }
    setLoading(false);
  };
          ...
          <Card style={[styles.card, styles.resultCard]} elevation={2}>
            ...
            <Card.Content>
              <Text variant="bodyLarge">
                Expected Return: {optimizationResult.expected_return.toFixed(2)}
                %
              </Text>
              <Text variant="bodyLarge">
                Volatility: {optimizationResult.volatility.toFixed(2)}%
              </Text>
              <Text variant="bodyLarge">
                Sharpe Ratio: {optimizationResult.sharpe_ratio.toFixed(2)}
              </Text>
              <List.Subheader style={styles.weightsTitle}>
                Optimal Weights
              </List.Subheader>
              <FlatList
