...
export const getPrediction = async (asset, timeframe, current_price) => {
  try {
    ...
    const isCrypto = ["BTC", "ETH", "XRP", "ADA", "SOL", "DOGE"].includes(
      asset.toUpperCase(),
    );
    const endpoint = isCrypto
      ? `/predictions/crypto/${asset}`
      : `/predictions/stocks/${asset}`;
...
export const getCoinMarketChart = (coinId, days = "7", vsCurrency = "usd") => {
  return axios
    ...
    .catch((error) => {
      ...
      return {
        data: generateMockMarketChartData(
          days,
          coinId === "bitcoin" ? 45000 : 3000,
        ),
      };
    });
};
...
export const getCryptoNews = async (page = 1, items = 20, tickers = "") => {
  try {
    ...
    if (storedToken && storedToken !== "YOUR_CRYPTONEWS_API_TOKEN") {
      ...
      console.warn(
        "CryptoNews API token not found or invalid. Using mock data.",
      );
      return {
        data: {
...
const generateMockCoinData = (coinId) => {
  ...
  const isBitcoin =
    coinId.toLowerCase().includes("btc") ||
    coinId.toLowerCase().includes("bitcoin");
...
const generateMockNewsData = (items = 20, page = 1, tickers = "") => {
  ...
  for (let i = 0; i < items; i++) {
    ...
    for (let j = 0; j < numTickers; j++) {
      const randomTicker =
        tickerList[Math.floor(Math.random() * tickerList.length)];
